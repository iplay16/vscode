...
int findDumplicateWord(char *s,int startIndex,int endIndex,char c,int *count){
    //int lengthbetweentwo;
    for(int i=startIndex+1;s[i]!='\0'&&i<endIndex;i++){
        if(s[i]==c){
            //lengthbetweentwo=endIndex-i;
            return i;
        }
    }
    *count=*count>endIndex-startIndex?*count:endIndex-startIndex;
    return startIndex;
}
...
int lengthOfLongestSubstring(char* s){
    ...
    for(int i=0;i<len;i++){
        // dflag=0;
        //findDumplicateWord(char *s,int startIndex,int endIndex,char c,int *count)
        dindex=findDumplicateWord(s,dindex,i,s[i],&count);//从上次重复位置开始寻找
